...
function sc_mask1Atk(){
	...
	if ( state == 10 ) {
		...
		if ( process == 55+lvDly ) { 
			...
			ob.image_xscale = dir;
			ob.sprite_index = sp_mask1_atkEf;
			ob.xSpeed = dir*2;
		}
		...
		else if ( process < 70+lvDly )	{ image_index = 8; }
		else if ( process < 75+lvDly )	{ image_index = 9; }
		else if ( process < 80+lvDly )	{ image_index = 10; }
