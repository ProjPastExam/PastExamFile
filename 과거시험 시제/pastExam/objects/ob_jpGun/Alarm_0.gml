...
if ( sc_getRoomValue("pause") == 0 ) {
	if ( state != -1 ) {
		...
		sprite_index = sp_jpGun_die;
		if ( process < 24 ) process++;
		if ( process == 1 ) { 
			var han = irandom_range(0, 2);
			if (han == 0) audio_play_sound(s_jpAdie1, 8, false);
			else if (han == 1) audio_play_sound(s_jpAdie2, 8, false);
			else audio_play_sound(s_jpAdie3, 8, false)
		}
